...
def main():
    ...
    image_urls = list(filter(lambda entry: entry.isFile and not entry.isSupplementary, data))
    ...
    supplementary_urls = list(filter(lambda entry: entry.isFile and entry.isSupplementary, data))
    supplementary_check_result = check(supplementary_urls, OUTPUT_SUPPLEMENTARIES_DIR, sha256sum_from_files)
